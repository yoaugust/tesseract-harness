...
- [Bug fix] Linux desktop `.deb` installs show the tesseract logo in the app launcher (#3396)
...
- [Bug fix] opencode-native no longer kills long-running relayed tesseract tool calls at 60 s; spec MCP server timeouts are honored correctly. (#5956)
...
- [Feature] Use the native tesseract app on iPad in portrait, landscape, and multitasking layouts. (#6173)
...
- [UI] Open About tesseract to see distinct app and CLI versions and manage desktop updates in one place. (#6178)
- [UI] Archive or unarchive a conversation straight from a sidebar row on hover, without opening the menu (#6179)
- [Bug fix] Local development sessions keep generating automatic titles after the tesseract source checkout changes. (#6180)
- [UI / Feature] Projects can now set a default model for Claude Code / Codex sessions in Project settings. (#6181)
- [UI] The onboarding "Starting tesseract" step now streams the local server's real startup logs instead of a spinner. (#6186)
...
- [Bug fix] tesseract now warns during setup and fails early at terminal launch when tmux 3.3 or newer is unavailable. (#6255)
...
- [Bug fix] The Working indicator now stays visible when an active Codex session survives an tesseract server restart. (#6669)
...
- [UI / Bug fix] Android notifications now use a legible tesseract creature icon in the status bar. (#4623)
...
- [Bug fix] tesseract now starts and runs migrations with database passwords that require URL encoding, including when operators use `OMNIGENT_DB_URL`. (#4960)
- [Chore / Test/CI] `deploy.py --no-otel` deploys tesseract to Databricks Apps without OpenTelemetry instrumentation or export (#5001)
...
- [Feature] `omnigent host` and `omnigent start` now open the tesseract web UI in your browser automatically (respects `auto_open_conversation`). (#5135)
...
- [UI] Corrected the setup screen to say "tesseract server" instead of "Omnigents server" (#5239)
...
- [Bug fix] Codex sessions now honor tesseract's configured retry count and request timeout. (#5378)
...
- [UI / Feature] Import your recent local Claude Code / Codex / other CLI sessions into tesseract (#5401)
...
- [UI / Bug fix] tesseract no longer shows a permanent update window in the macOS Dock and reliably finishes quitting. (#5413)
...
- [UI / Feature] Administrators can provide tesseract Desktop server choices through macOS Managed Preferences. (#5467)
...
- [Feature / Docs] Automatic session titles can now follow custom formatting instructions from tesseract configuration. (#5532)
...
- [Bug fix / Feature] `omni setup` now offers a "Pi — use own login" credential option, letting Pi users with local Llama, Google, or other natively-configured providers bypass tesseract-managed authentication. (#5715)
- [UI / Bug fix / Feature] Embedded tesseract now applies UI font size and the color palette, and can show a host-provided theme-settings link. (#5721)
...
- [Feature] The desktop app now offers to take you back to your tesseract server when a sign-in flow navigates the window away and doesn't return (#5773)
...
- [UI / Bug fix] Chat links now support normal navigation, new-tab, and copy-link behavior while workspace files still open inside tesseract. (#5789)
...
- [UI / Bug fix / Docs] Sign in to Google-backed tesseract servers from the iOS app without hitting `disallowed_useragent` (#2556)
...
- [Feature] Renaming a session with `/rename` inside a native Claude Code terminal now updates its name in the tesseract UI (#4104)
...
- [Bug fix] The Electron app now returns you to tesseract when a Databricks workspace navigation lands at the workspace root. (#5125)
...
- [Bug fix] Older tesseract clients now explain when a database was migrated by a newer version instead of recommending an impossible schema upgrade. (#5377)
...
- [UI / Feature / Docs / Test/CI] White-label the web UI from server config with custom names, headings, safe logo assets, favicon, and optional tesseract attribution. (#2857)
...
- [UI / Feature] Organizations can preconfigure tesseract server URLs through Android managed configuration, and they show up ready to tap in the app's server list. (#4315)
...
- [Bug fix] The Android app no longer shows the Databricks workspace navigation bar around tesseract when connecting to a workspace-hosted server. (#4555)
...
- [UI] Connecting the iOS app to a Databricks workspace now opens tesseract directly and hides the workspace navigation bar (#4559)
...
- [Chore] Removed the "A new version of tesseract is available" prompt and browser PWA install support; the desktop and mobile apps remain the installable clients. (#4617)
...
- [Feature / Docs] ArgoCD quick-start overlay for deploying tesseract with the kubernetes sandbox provider (#4788)
...
- [Docs / Chore] Resuming a goose / hermes / antigravity / qwen / opencode native session now attaches to the live TUI instead of double-posting each message through the tesseract REPL. (#3314)
...
- [Bug fix / Feature / Docs / Test/CI] ACP agents can opt out of tesseract’s MCP relay with `omnigent_mcp: false`, enabling compatible OpenClaw Gateway ACP registrations. (#3420)
...
- [Bug fix] `/model` and the startup header no longer name an tesseract provider and model for ACP-backed sessions, which run on the agent's own auth and model. (#3431)
...
- [UI / Chore] `omnigent --help` command descriptions are tidied (harness rows read "Launch <Name> with tesseract"), and the duplicate `update` alias line is hidden from the listing (#3797)
...
- [Bug fix] Pi-native sessions now retain tesseract system tools and the comment relay. (#3920)
...
- [Feature] Import local Qwen, Kiro, Pi, and Kimi coding chats into tesseract (#3032)
...
- [Bug fix / Feature] The Slack bot can now run against an tesseract server deployed on Databricks Apps, (#3051)
...
- [Bug fix] Fixed the headless Hermes harness registering no tesseract MCP server (agent had no builtin tools). (#2216)
...
- [Feature / Docs / Test/CI] Harness bench now reports tesseract MCP relay support separately from vendor-native tool calling. (#2380)
...
- [Bug fix] Pi no longer shows a blocking "Trust project folder?" dialog when launched via tesseract in a project with `.pi/` settings or extensions (requires Pi 0.79+) (#2529)
...
- [Feature / Docs / Test/CI] Add `omnigent uninstall` for safely removing the CLI, installer-managed shell/config edits, and optionally local tesseract state. (#2550)
...
- [UI / Bug fix] The file editor now follows tesseract's selected web and desktop color theme instead of the operating-system theme. (#2594)
...
- [Bug fix] Clicking an tesseract session URL printed inside the embedded terminal now stays in the web app instead of opening a duplicate browser tab/window. (#2639)
...
- [UI / Bug fix] "Back to tesseract" from Settings now returns you to the conversation you were viewing instead of the home page (#1905)
...
- [UI / Feature] Pick a color theme (tesseract, Dracula, GitHub, Catppuccin, or Gruvbox) in Appearance settings, independent of light/dark mode. (#2147)
...
- [Bug fix] Fixed the Hermes `pre_tool_call` hook double-gating tesseract relay tools, which parked a (#2220)
