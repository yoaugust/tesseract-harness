...
This test runs a REAL tesseract server subprocess against a REAL SQLite
...
1. The store is constructed in a separate process from the
   ...
   where the tesseract server is a subprocess of the REPL).
...
- Spins up an tesseract server subprocess (:fixture:`ap_server`).
- Imports the tesseract server's actual ``SqlAlchemyConversationStore``
...
def _find_free_port() -> int:
    """Pick a free TCP port for the tesseract subprocess to bind."""
    s = socket.socket()
    s.bind(("", 0))
...
def test_concurrent_appends_against_live_omnigent_server_db_no_collision(
    ...
    """
    With a live tesseract server running, fire N concurrent appends
    ...
    - The lock-escalation fix in
      ...
      (the tesseract server). On revert, the test surfaces the user's
    ...
    Note: this test does NOT exercise the tesseract route layer (the
    ...
    # this gets a separate engine in this process (the tesseract server
    ...
    assert errors == [], (
        f"Concurrent appends against the live tesseract server's SQLite "
