...
describe("UpdateBanner", () => {
  ...
  it("renders the correct controls for available, downloading, and downloaded states", async () => {
    ...
    expect(await screen.findByText("tesseract Desktop 0.4.0 is available")).toBeInTheDocument();
    ...
    expect(await screen.findByText("Downloading tesseract Desktop update… 42%")).toBeInTheDocument();
    ...
    expect(
      await screen.findByText("tesseract Desktop 0.4.0 is ready to install"),
    ).toBeInTheDocument();
    expect(screen.getByText("Current version: 0.3.0.")).toBeInTheDocument();
    expect(screen.getByText("Updating won’t interrupt existing sessions.")).toBeInTheDocument();
  ...
  it("suppresses a skipped version after persisting it", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText("tesseract Desktop 0.4.0 is available")).toBeNull();
    });
  });
});
