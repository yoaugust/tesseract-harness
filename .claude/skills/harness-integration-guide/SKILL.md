...
description: Reference guide for building new tesseract harness integrations — covers SDK/subprocess harnesses and native harnesses as separate tracks, each with their own feature matrix, implementation patterns, and prioritized checklist.
---

# Harness integration guide

This skill describes the **feature matrix** every tesseract harness must
consider. Use it when planning, reviewing, or implementing a new harness.

tesseract has two distinct harness tracks with different architectures and
feature sets:
...
- **Native harnesses** — wrap a vendor's own TUI or server and mirror its
  output into tesseract. They observe and relay, rather than drive.

---

## Part 1 — SDK / subprocess harnesses

These harnesses run the vendor model directly and bridge tesseract tools into
...
|---|---|
| **Connects to tesseract MCP** | Harness exposes/consumes tools via the MCP protocol (in-proc SDK MCP server) |
...
| **Streaming** | Harness forwards token-level or delta-level streaming to the tesseract forwarder |
| **tesseract policies** | Harness enforces tesseract-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the harness surfaces the approval request in the tesseract web UI so the user can approve or deny |
...
| **Tool-boundary steer** | tesseract can inject steering text at tool-call boundaries |
| **Resume/fork from tesseract transcript** | Rebuild a conversation from a stored tesseract transcript (replay history, seed prompt, or vendor session ID) |
...
| **Cost tracking** | Harness reports token usage and cost data back to tesseract for each turn |

### MCP connectivity

The harness must bridge tesseract's builtin MCP tools so the model can call
...
The harness must support the tesseract policy engine's three verdicts at two
checkpoints:
...
or tool result in the tesseract web UI as an approval card, then relay the
...
| Vendor session ID | Relies on the vendor's own session persistence (no tesseract-side rebuild) |
...
- [ ] Connects to tesseract MCP (in-proc SDK MCP server or vendor-specific bridge)
- [ ] Model override works (or document vendor lock-in)
- [ ] Auth is configured and documented (setup flow in `omni setup`)
- [ ] Streaming forwards to the tesseract forwarder
- [ ] tesseract policies enforce tool-use rules
...
- [ ] Resume/fork rebuilds conversation from tesseract transcript
...
tesseract. They relay the vendor's conversation into the tesseract session.
...
| **Connects to tesseract MCP** | Whether the native harness connects to the tesseract MCP server |
...
| **tesseract policies** | Whether the native harness enforces tesseract-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the native harness surfaces the approval request in the tesseract web UI so the user can approve or deny |
| **Interrupt** | User can abort a running turn |
| **Bidirectional sync (TUI->Omni)** | TUI output mirrors into the tesseract conversation |
| **In-harness session-cmd sync** | Supports `clear`, `fork`, `resume`, `switch` commands from tesseract |
| **Resume/fork from tesseract transcript** | Can rebuild conversation from tesseract transcript (native rebuild, or fresh launch) |
...
| **Cost tracking** | Native harness reports token usage and cost data back to tesseract for each turn |
...
- [ ] Connects to tesseract MCP
- [ ] Auth configured (vendor login / config)
- [ ] Streaming forwarder works (deltas preferred; complete-only acceptable)
- [ ] tesseract policies enforce tool-use rules (ALLOW / ASK / DENY at both tool call and tool result)
- [ ] Native elicitation surfaces tool-approval requests to web UI
- [ ] Interrupt aborts the running turn
- [ ] Bidirectional sync mirrors TUI output into tesseract conversation
- [ ] Cost tracking reports token usage and cost per turn
...
- [ ] Session commands (clear, fork, resume) work from tesseract
- [ ] Resume/fork rebuilds from tesseract transcript
- [ ] Reasoning tokens are forwarded
