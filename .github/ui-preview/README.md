...
Deploy a live, per-PR preview of the tesseract web UI as a
...
2. The [UI Preview workflow](../workflows/ui-preview.yml) builds the SPA + the
   tesseract wheels and deploys them to an ephemeral Databricks App
...
Unlike tesseract's production Databricks deploy (`deploy/databricks/`, backed by
...
There is **no LLM or runner baked into the preview** -- tesseract runs agent
