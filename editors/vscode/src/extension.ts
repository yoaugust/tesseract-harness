/**
 * tesseract VS Code extension entry point (minimal iframe-only build).
 *
 * activate() wires:
 *  - Config / local-server discovery
 *  - A minimal Sessions/home tree view (so the activity-bar icon renders) whose
 *    welcome content offers an "Open tesseract" button
 ...
 * call-to-action is the `viewsWelcome` "Open tesseract" button in package.json.
 */
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  output = vscode.window.createOutputChannel("tesseract");
