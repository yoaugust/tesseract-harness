...
# Translates tesseract spec executor types (underscore) to harness subprocess names
...
def _get_runner_client_for_compaction(
    ...
    profile) instead of the tesseract server's.
...
_HARNESS_DATABRICKS_PROFILE: dict[AgentHarnessType, str] = {
    ...
    # so tesseract cannot thread a Databricks gateway through. Users configure
    # providers via ``kimi provider add`` in ``~/.kimi-code/config.toml``
    # (tesseract-side provider injection is a deferred follow-up).
}
...
def _build_claude_sdk_spawn_env(
    ...
    *,
    ...
    design doc), per-spawn env overrides are how tesseract threads
    ...
    # some configurations); routing through tesseract mode without a
    # similar default leaves tesseract mode users staring at a
    ...
    # active and approval requests surface via tesseract elicitation. Read from
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    on the tesseract server directly (they propagate to the subprocess
    ...
    *,
    ...
    if spec.executor.auth is not None:
        raise OmnigentError(
            ...
            "resulting model id in the agent spec. tesseract-side provider "
...
def _load_initial_history(
    ...
    # the empty synthetic pair — the tesseract thinks context is ~0 tokens
...
def _maybe_persist_compaction_item(
    ...
    # session while the tesseract thinks context is near-zero and never
