...
``wake_dispatch`` coroutine (kept free of HTTP/server knowledge) wired up by the tesseract server
...
class SubagentBlockNotifier:
    ...
    One instance is constructed and registered per tesseract server process.
    ...
    :param loop: The tesseract server's event loop, captured at registration
