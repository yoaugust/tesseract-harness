"""Sandbox CLI commands: run an tesseract host in a remote sandbox.

``omnigent sandbox …`` bootstraps an tesseract host inside a sandbox
...
def sandbox() -> None:
    """
    Run an tesseract host inside a remote sandbox.

    \b
    Subcommands:
      create   Provision a sandbox + bootstrap tesseract into it.
...
def sandbox_create(
    ...
    """
    Provision a sandbox and ship tesseract into it.
    ...
    profile. Builds the tesseract wheels from your local checkout,
