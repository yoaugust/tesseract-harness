...
- ``--server <url>``: connect to an existing (remote or local) tesseract server.
- ``--local``: this daemon owns a local tesseract server — start (or reuse) a
...
def main() -> None:
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Start (or reuse) a local tesseract server and connect to it.",
    )
