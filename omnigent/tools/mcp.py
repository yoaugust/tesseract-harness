...
class McpServerConnection:
    ...
    async def _invoke_tool(
        ...
        ``/mcp/execute`` → tesseract server) can surface the elicitation
    ...
    async def call_tool_with_elicitation(
        ...
        Called by the tesseract server after the user has responded to an
        ...
        # InputRequiredResult on the retry. Raise so the tesseract server
    ...
    async def _open_stdio_transport(
        ...
        (``omnigent/inner/mcp_tools.py``); tesseract now matches that
...
async def _call_tool_with_reconnect(
    ...
    :param session_id: tesseract session id forwarded to
