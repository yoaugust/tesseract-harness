...
class Elicitation(str, Enum):
    """How a policy ASK / tool-approval is surfaced to the tesseract web UI."""

    NONE = "none"
    HOOK = "hook"  # vendor PreToolUse hook posts to tesseract
...
class Resume(str, Enum):
    ...
    COLD_ONLY = "cold-only"  # rebuild from tesseract transcript / history replay
...
class AuthModel(str, Enum):
    ...
    OMNIGENT_CREDENTIAL = "omnigent-credential"  # tesseract gateway / provider config
    OWN_AUTH = "own-auth"  # vendor login / API key, not tesseract-managed
...
class HarnessCapabilities:
    ...
    :param subagents: Whether the harness can spawn tesseract sub-agents.
